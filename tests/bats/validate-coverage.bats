...
@test "full coverage exits 0" {
    run bash "$SCRIPTS_DIR/validate-coverage.sh" "$FIXTURES_DIR/minimal"
    assert_success
}

@test "full coverage JSON shows has_gaps false" {
    run bash "$SCRIPTS_DIR/validate-coverage.sh" --json "$FIXTURES_DIR/minimal"
    assert_success
    local val
...
@test "missing ATP exits 1" {
    run bash "$SCRIPTS_DIR/validate-coverage.sh" "$FIXTURES_DIR/gaps"
    assert_failure
}

@test "missing ATP identifies REQ-NF-001" {
    run bash "$SCRIPTS_DIR/validate-coverage.sh" --json "$FIXTURES_DIR/gaps"
    assert_failure
...
@test "empty files handled" {
    run bash "$SCRIPTS_DIR/validate-coverage.sh" --json "$FIXTURES_DIR/empty"
    assert_success
    local val
    val=$(echo "$output" | python3 -c "import json,sys; print(json.load(sys.stdin)['total_reqs'])")
    [ "$val" = "0" ]
