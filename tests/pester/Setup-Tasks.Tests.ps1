...
# Pester mirror of tests/bats/setup-tasks.bats — exercises the
# scripts/powershell/setup-tasks.ps1 bridge wrapper with a stub upstream
# injected via $env:SPECIFY_REPO_ROOT.

BeforeAll {
    $script:Wrapper = Resolve-Path (Join-Path $PSScriptRoot '../../scripts/powershell/setup-tasks.ps1')
...
@{ FEATURE_DIR = '$($FeatureDir -replace "'", "''")'; AVAILABLE_DOCS = @('research.md') } | ConvertTo-Json -Compress
exit $ExitCode
"@
...
Describe 'setup-tasks.ps1 bridge wrapper' {
    BeforeEach {
