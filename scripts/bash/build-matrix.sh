...
# Allocate same-directory temp files (collision-resistant) to avoid the
# /tmp race that occurs when two concurrent invocations share fixed paths.
# Cleaned up via trap so failures don't leak artifacts.
if [[ -n "$OUTPUT" ]]; then
    TMPDIR_BUILD="$(dirname "$(realpath -m "$OUTPUT")")"
else
    TMPDIR_BUILD="$VMODEL_DIR"
fi
matrix_body="$(mktemp "$TMPDIR_BUILD/.vmodel-matrix-body.XXXXXX")"
matrix_full="$(mktemp "$TMPDIR_BUILD/.vmodel-matrix-full.XXXXXX")"
trap 'rm -f "$matrix_body" "$matrix_full"' EXIT INT TERM
