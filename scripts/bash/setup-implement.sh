...
# augments the upstream JSON with a top-level VMODEL_DIR field. Distinct file
# from setup-tasks.sh so per-command frontmatter wiring stays one-to-one.
# Repo root overridable via SPECIFY_REPO_ROOT.
set -euo pipefail
IFS=$'\n\t'
