<#
.SYNOPSIS
    PowerShell mirror of scripts/bash/setup-tasks.sh — bridge wrapper around
    .specify/scripts/powershell/check-prerequisites.ps1 that augments the
    upstream JSON with a top-level VMODEL_DIR field.
...
.NOTES
    ...
    -IncludeTasks -PathsOnly switch names (PascalCase mirror of bash flags).
    The upstream PS variant is not present in this checkout.
#>
[CmdletBinding()]
param(
    [switch]$Json,
