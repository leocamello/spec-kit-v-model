<#
.SYNOPSIS
    PowerShell mirror of scripts/bash/setup-plan.sh — bridge wrapper around
    .specify/scripts/powershell/setup-plan.ps1 that augments the upstream
    JSON with a top-level VMODEL_DIR field.
...
.NOTES
    ...
    Assumption: upstream PS script is invocable with -Json and emits a JSON
    object whose feature-dir field is named "SPECS_DIR" (matching the bash
    contract); the upstream PS variant is not present in this checkout, so
    we mirror the bash field name conservatively.
#>
[CmdletBinding()]
param(
    [switch]$Json,
...
$Upstream = Join-Path $RepoRoot '.specify/scripts/powershell/setup-plan.ps1'
...
if ($Json) {
    ...
    $featureDir = $null
    foreach ($name in @('SPECS_DIR', 'FeatureDir', 'SpecsDir', 'FEATURE_DIR')) {
    ...
    foreach ($line in ($joined -split "`n")) {
        if ($line -match '^(SPECS_DIR|FEATURE_DIR):\s*(.+)$') { $featureDir = $Matches[2].Trim(); break }
    }
    if ($featureDir -and (Test-Path -LiteralPath (Join-Path $featureDir 'v-model') -PathType Container)) {
