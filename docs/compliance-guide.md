...
Our helper scripts (`validate-requirement-coverage.sh`, `validate-system-coverage.sh`, `validate-architecture-coverage.sh`, `validate-module-coverage.sh`, `validate-hazard-coverage.sh`, `build-matrix.sh`) use regex pattern matching to:
...
5. Ensure all source files (`requirements.md`, `acceptance-plan.md`, `hazard-analysis.md`, etc.) are committed to version control
