...
| **REQ-IF-004** | The `validate-system-coverage.sh` script SHALL output a structured coverage summary to stdout in the same format as `validate-coverage.sh` (section headers, gap lists, pass/fail verdict, coverage percentages). | ATP-IF-004-A | Consistent Format with validate-coverage.sh | SCN-IF-004-A1 | ⬜ Untested |
| **REQ-NF-001** | The `validate-system-coverage.sh` script SHALL use regex-based parsing consistent with `validate-coverage.sh` from v0.1.0, requiring no runtime database or external tooling beyond standard Bash utilities. | ATP-NF-001-A | Script Uses Only Bash Builtins and Standard Utilities | SCN-NF-001-A1 | ⬜ Untested |
