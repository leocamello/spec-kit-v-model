...
| REQ-IF-003 | The `v-model.implement` command SHALL register the `before_implement` and `after_implement` extension hooks to invoke the `v-model.trace` command. | P1 | Hook registration on the implement side is what wires the bridge command into the existing pre/post-implementation automation graph; without it `v-model.trace` would not run automatically around implementation. (Source: spec.md FR-028; split from prior compound REQ-IF-003 per PRF-REQ-001) | Test |
...
| REQ-IF-005 | The `/speckit.v-model.requirements` command SHALL be reachable via the `after_specify` hook. | P1 | The `after_specify` hook entry is what kicks off the V-cycle automatically once `/speckit.specify` completes; without it the requirements step would have to be invoked manually. (Source: spec.md FR-028; split from prior compound REQ-IF-003 per PRF-REQ-001) | Test |
